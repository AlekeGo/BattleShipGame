...
class GamesRepo:
    ...
    async def create(self, user_id: str, mode: str, bot_ships: list[dict]) -> dict:
        payload = {
            "user_id": user_id,
            "mode": mode,
            "player_board": {"ships": [], "shots_received": []},
            "bot_board": {"ships": bot_ships, "shots_received": []},
            "moves": [],
            "status": "placing",
        }
    ...
    async def update_state(self, game_id: str, **fields) -> None:
        self.db.table("games").update(fields).eq("id", game_id).execute()
    ...
    async def append_move(self, game_id: str, move: dict) -> None:
        ...
        moves.append(move)
        self.db.table("games").update({"moves": moves}).eq("id", game_id).execute()
